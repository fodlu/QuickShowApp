...
const AddShows = () => {
  ...
  const [selectMovies, setSelectMovies] = useState(null);
  ...
  const handleSubmit = async ()=> {
    try {
      setAddingShow(true)
      if(!selectMovies || Object.keys(dateTimeSelection).length === 0 || !showPrice) {
  ...
  return (
    <>
      ...
      <div className="overflow-x-auto pb-4">
        <div className="group flex flex-wrap gap-4 mt-4 w-max">
          {nowPlayingMovies.map((movie)=>(
            <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:translate-y-1 transition duration-300`} onClick={()=>setSelectMovies(movie.id)}>
      ...
      <button className="bg-primary text-white px-8 py-2 mt-6 rounded hover:bg-primary/90 transition-all cursor-pointer" onClick={handleSubmit} disabled={addingShow}>Add Show</button>
    </>
  )
}
